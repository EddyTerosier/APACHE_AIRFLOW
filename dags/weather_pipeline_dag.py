...
Pipeline météo quotidien pour 3 villes (Paris, Lyon, Marseille).
Utilise l'API Open-Meteo (gratuite, sans clé API).

Workflow :
    fetch_weather  →  validate_weather  →  load_weather

Exemple de réponse Open-Meteo pour Paris :
{
  "current": {
    "temperature_2m": 18.4,
...
def validate_weather() -> None:
    ...
    Note : en production, les données seraient lues depuis un stockage
    intermédiaire (XCom, BDD staging). Ici on re-appelle l'API pour
    rester dans les concepts vus en cours.
    """
...
def load_weather() -> None:
    ...
    En production : remplacer les logger.info par de vrais INSERT
    via psycopg2, SQLAlchemy ou un PostgresHook Airflow.
